...
// A Graphite package forked off https://github.com/peterbourgon/g2g
// Credit to Peter Bourgon for the initial work
...
type Graphite struct {
	...
	endpoint	string			
	// host:port pair for the Carbon server
	interval	time.Duration		
	// how often do we write to the Carbon server
	timeout		time.Duration		
	// how long will we wait to connect/write
	retries		int			
	// how many times will we retry
	connection	net.Conn		
	// our net.Conn
	vars		map[string]interface{}	
	// key:value pairs in an accessible interface
	registers	chan namedvars		
	// channel for registering vars
	shutdown	chan chan bool		
	// control channel for closing out or Graphite structure cleanly
	debug		bool			
	// enable debug
}
...
// 
// Example
// g = graphite.NewGraphite("Localhost:2003", 10 * time.Second, 2 * time.Second, 3, false)
// returns a Graphite object which writes to the Carbon server running on the localhost on port 2003
// at 10 second intervals with a 2 second timeout to writes and no debugging. It will attempt to reconnect
// to the Carbon server 3 times before failing out entirely.
...
// It is acceptable to call PostOne directly if your program will exit on completion so you do not need to hang around waiting for the loop to execute
